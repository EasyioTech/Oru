...
> **Note**: System settings PUT was refactored separately. It now uses `backend/src/services/systemSettingsService.js`; `ensureSystemSettingsSchema` is no longer used (schema is migration-only). See `frontend/src/docs/SYSTEM_SETTINGS.md`.
