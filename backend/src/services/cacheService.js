...
// One-time warning when using in-memory fallback (avoids log spam)
let memoryFallbackWarned = false;
function warnMemoryFallbackOnce(operation) {
  if (!memoryFallbackWarned) {
    memoryFallbackWarned = true;
    logger.warn(
      '[Cache] Redis unavailable: using in-memory fallback. Sessions and cache will not be shared across instances or survive restarts.',
      { operation }
    );
  }
}
...
async function get(key) {
  ...
  try {
    ...
    logger.warn('[Cache] Redis get error, falling back to memory', { message: error.message });
  }
  if (!usedRedis) {
    warnMemoryFallbackOnce('get');
  }
...
async function set(key, value, ttl = DEFAULT_TTL) {
  try {
    ...
    logger.warn('[Cache] Redis set error, falling back to memory', { message: error.message });
  }
  warnMemoryFallbackOnce('set');
...
async function del(key) {
  try {
    ...
    logger.warn('[Cache] Redis delete error, falling back to memory', { message: error.message });
  }
  warnMemoryFallbackOnce('del');
