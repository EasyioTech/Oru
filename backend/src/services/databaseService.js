...
const { ensureAgencySchema } = require('../infrastructure/database/schemaValidator');
...
async function repairMissingColumn(agencyDatabase, tableName, columnName) {
  ...
  try {
    ...
    } else if (tableName === 'profiles' && columnName === 'personal_email') {
      await agencyClient.query('ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS personal_email TEXT');
      console.log(`[API] ✅ Added ${columnName} column to ${tableName} table`);
