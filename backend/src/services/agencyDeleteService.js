...
async function deleteAgency(agencyId) {
  ...
  try {
    ...
    // Step 0: Remove agency pool from pool manager so we don't leave a dead pool in cache
    try {
      const { removeAgencyPool } = require('../infrastructure/database/poolManager');
      removeAgencyPool(databaseName);
    } catch (poolErr) {
      console.warn('[Delete] Could not remove pool from cache:', poolErr.message);
    }
