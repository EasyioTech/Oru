...
async function ensureDepartmentsTable(client) {
  ...
  `);
  await ensureDepartmentsTableConstraints(client);
}

/**
 * Add UNIQUE(name), CHECK(budget >= 0) to departments (idempotent).
 * Hierarchy is per-DB so one name per database.
 */
async function ensureDepartmentsTableConstraints(client) {
  try {
    await client.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS idx_departments_name_unique ON public.departments(name);
    `);
  } catch (e) {
    if (e.code !== '23505') throw e;
    console.warn('[SQL] Departments: unique index on name skipped (duplicate names exist); resolve duplicates then re-run.');
  }
  await client.query(`
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'chk_departments_budget_non_negative' AND conrelid = 'public.departments'::regclass
      ) THEN
        ALTER TABLE public.departments ADD CONSTRAINT chk_departments_budget_non_negative CHECK (budget >= 0);
      END IF;
    END $$;
  `);
}
...
async function ensureTeamAssignmentsTable(client) {
  ...
  await client.query(`
    ...
    END $$;
  `);
  await ensureTeamAssignmentsDepartmentFK(client);
}

/**
 * Add FK team_assignments.department_id -> departments(id) ON DELETE SET NULL (idempotent).
 */
async function ensureTeamAssignmentsDepartmentFK(client) {
  await client.query(`
    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_team_assignments_department' AND conrelid = 'public.team_assignments'::regclass
      ) THEN
        ALTER TABLE public.team_assignments
          ADD CONSTRAINT fk_team_assignments_department
          FOREIGN KEY (department_id) REFERENCES public.departments(id) ON DELETE SET NULL;
        CREATE INDEX IF NOT EXISTS idx_team_assignments_department_id ON public.team_assignments(department_id);
      END IF;
    END $$;
  `);
}
