...
/**
 * Repair orphaned employees: create employee_details for users with profiles but no employee_details.
 * These occur when employee creation partially failed. Run after unified_employees view exists.
 */
async function repairOrphanedEmployees(client) {
  try {
    const result = await client.query(`
      ...
      numbered AS (
        SELECT o.*, nn.n + (row_number() OVER ()) - 1 AS emp_num
        FROM orphans o
        CROSS JOIN next_num nn
      )
      ...
      SELECT
        gen_random_uuid(),
        n.user_id,
        'EMP-' || LPAD(n.emp_num::TEXT, 4, '0'),
        n.agency_id,
        COALESCE(NULLIF(TRIM(SPLIT_PART(COALESCE(n.full_name, 'Unknown User'), ' ', 1)), ''), 'Unknown'),
        COALESCE(TRIM(SUBSTRING(COALESCE(n.full_name, ' ') || ' ' FROM (LENGTH(SPLIT_PART(COALESCE(n.full_name, ' '), ' ', 1)) + 2))), 'User'),
        'full_time',
        NULL,
    ...
    if (count > 0) {
      console.log(`[SQL] ✅ Repaired ${count} orphaned employee(s)`);
    }
  } catch (error) {
    console.warn('[SQL] ⚠️ Orphaned employee repair failed (non-fatal):', error.message);
  }
}

module.exports = {
  ensureHrSchema,
  repairOrphanedEmployees,
