...
- **Main database (`oru_erp`)**: Control plane only. Tenant catalog, platform config, page catalog, system identity. See [Main DB Multi-Tenant Plan](.cursor/plans/) for the 12 tables.
...
This creates the `oru_erp` database. The backend will create tables automatically on first start (from `database/migrations/01_core_schema.sql`).
...
- **User:** postgres  
- **Password:** admin  
- **URL:** `postgresql://postgres:admin@localhost:5432/oru_erp`
...
```bash
psql -U postgres -h localhost -c "CREATE DATABASE oru_erp;"
```
