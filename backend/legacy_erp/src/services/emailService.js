...
async function sendEmailViaMailtrap(mailOptions, config = {}) {
  ...
  const apiToken = config.apiToken || process.env.MAILTRAP_API_TOKEN;
  const from = mailOptions.from || process.env.MAILTRAP_FROM || 'noreply@oru.app';
...
async function sendEmail(to, subject, html, text = null, attachments = [], options = {}) {
  try {
    ...
    if (provider === PROVIDERS.POSTMARK) {
      const mailOptions = {
        from: options.from || process.env.POSTMARK_FROM || 'noreply@oru.app',
        to,
        subject,
        html,
        text: text || html.replace(/<[^>]*>/g, ''),
    ...
    if (provider === PROVIDERS.MAILTRAP) {
      const mailOptions = {
        from: options.from || process.env.MAILTRAP_FROM || 'noreply@oru.app',
        to,
        subject,
        html,
        text: text || html.replace(/<[^>]*>/g, ''),
