...
- **Main database (`oru_erp`)**: Control plane only. Tenant catalog (agencies table: domain → database_name), platform config (system_settings, page_catalog), system identity (users, profiles, user_roles for super_admin). 12 tables total. See `database/README.md` and plan in `.cursor/plans/`.
