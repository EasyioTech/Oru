...
> **Note (System Settings)**: `ensureSystemSettingsSchema` and `verifySchemaExists` have been **removed** from the codebase. System settings schema is now managed **only by database migrations** (`database/migrations/12_system_settings_schema.sql`). See `frontend/src/docs/SYSTEM_SETTINGS.md` for current architecture.
...
- ~~Line 2789/2805/2917: `ensureSystemSettingsSchema` / `verifySchemaExists`~~ **REMOVED** – schema is migration-only now.
