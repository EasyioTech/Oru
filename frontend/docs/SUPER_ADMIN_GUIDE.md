...
You can log in as super admin in two ways:

#### Option A: Standard auth page (with empty Workspace)

1. Navigate to: `http://localhost:8080/auth`
2. Enter credentials:
   - **Email**: `super@buildflow.local`
   - **Password**: `super123`
   - **Workspace**: Leave blank (super admin uses main DB only)
3. Click "Sign In"

#### Option B: Dedicated secret system-admin login (recommended for production)

A **secret, dedicated** login path is available for system (super) admin only. It is **not linked** from the main site (auth, landing, footer). Use it only by direct URL or bookmark.

1. Navigate directly to the secret path (default: `http://localhost:8080/sauth`)
   - Path is configurable via env: `VITE_SAUTH_PATH` (e.g. `/sauth` or `/sys-login`)
2. Enter **email** and **password** only (no Workspace field)
3. On success you are redirected to `/super-admin`

**Optional subdomain:** If you set `VITE_SAUTH_SUBDOMAIN=sauth`, visiting `https://sauth.yourdomain.com/` will redirect to the secret path (e.g. `/sauth`). DNS must point that subdomain to the same app.

**Security:** Do not link to `/sauth` (or the subdomain) from the main auth page, landing, footer, or any public navigation. Do not add it to sitemaps or public docs.

### Step 3: Access System Dashboard

After login, navigate to: `http://localhost:8080/super-admin` or `http://localhost:8080/system`
...
| Action | URL | Method |
|--------|-----|--------|
| Login (agency or super admin) | `/auth` | GET |
| Secret system-admin login (no links; direct URL only) | `/sauth` or `VITE_SAUTH_PATH` | GET |
| System Dashboard | `/super-admin` or `/system` | GET |
