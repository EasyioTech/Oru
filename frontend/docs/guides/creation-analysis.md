...
> **Current implementation (as of this project):** POST `/api/agencies/create` returns **202 Accepted** with `jobId`; poll **GET `/api/agencies/provisioning/:jobId`** for `status` (`pending`|`running`|`completed`|`failed`) and `result`/`error`. Idempotency via `Idempotency-Key` header. See `docs/ARCHITECTURE.md` and `backend/src/routes/agencies.js`.

**1. Progress Tracking**
```javascript
// WebSocket or Server-Sent Events for progress updates (conceptual; live API uses provisioning/:jobId)
...
// Progress endpoint (live: GET /api/agencies/provisioning/:jobId)
