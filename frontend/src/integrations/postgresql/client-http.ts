    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (isDatabaseNotFound && !isSchemaError) {
          ...
          console.warn('[HTTP DB] Schema error (expected during migrations):', msg || err);
        }
...
export async function transaction<T>(
  ...
  try {
    ...
    const userId = typeof window !== 'undefined' ? window.localStorage.getItem('user_id') || undefined : undefined;
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error('Request timed out. The operation took too long. Please try again.');
    }
    throw error;
  }
}
