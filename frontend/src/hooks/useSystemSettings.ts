...
import { fetchSystemSettings, type SystemSettings } from '@/services/api/system';
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      try {
        ...
        // Favicon is applied by BrandingProvider (fetchBranding) for all users
        ...
        setError(err instanceof Error ? err : new Error('Failed to load system settings'));
      } finally {
        setLoading(false);
      }
    };
