        ...
        <div className="space-y-10">
            ...
            <div className="space-y-3">
                ...
                <div className="grid gap-2.5">
                  ...
                  ${hasBusinessSelection && !isSelected ? 'opacity-40 blur-[0.5px]' : ''}
                `}
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-300 ${isSelected ? 'bg-foreground text-background' : 'bg-muted/60 text-muted-foreground'}`}>
                                    <Icon className="w-[18px] h-[18px]" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className={`text-[13px] font-semibold transition-colors ${isSelected ? 'text-foreground' : 'text-foreground/70'}`}>{model.label}</div>
                                    <div className="text-[11px] text-muted-foreground/60 mt-0.5">{model.description}</div>
                                </div>
            ...
            <AnimatePresence>
                {showTeamSize && (
                    ...
                    >
                      ...
                      ${hasTeamSelection && !isSelected ? 'opacity-40 blur-[0.5px]' : ''}
                    `}
                                    >
                                        <div className={`text-xl font-bold font-display tracking-tight transition-colors ${isSelected ? 'text-foreground' : 'text-foreground/60'}`}>{size.label}</div>
                                        <div className="text-[11px] text-muted-foreground/60 mt-0.5">{size.sub}</div>
                                        <div className={`text-[10px] font-medium mt-2 transition-colors ${isSelected ? 'text-foreground/50' : 'text-muted-foreground/40'}`}>{size.tag}</div>
                                    </motion.button>
                                );
                            })}
                        </div>
