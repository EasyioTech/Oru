  ...
  useEffect(() => {
    if (agencySettingsData) {
      ...
      setAgencySettings({
        ...
        primary_color: agencySettingsData.primary_color || '#0a6ed1',
        secondary_color: agencySettingsData.secondary_color || '#0854a0',
    ...
    try {
      ...
      if (logoFile) {
        try {
          ...
          const pathWithinBucket = `agency_${Date.now()}.${fileExt}`;
      ...
      if (result.success) {
        if (logoFile && settingsToSave.logo_url) {
          setAgencySettings(prev => ({ ...prev, logo_url: settingsToSave.logo_url }));
          setLogoPreview(settingsToSave.logo_url);
        }
        setLogoFile(null);
