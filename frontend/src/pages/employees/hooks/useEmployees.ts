...
export const useEmployees = (urlDepartmentId?: string | null) => {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      // Use unified_employees view with limit for scalability
      ...
      const unifiedEmployees: UnifiedEmployee[] = unifiedData.map((emp: any) => {
        ...
        return {
          ...
          full_name: emp.display_name || emp.full_name || 'Unknown User',
  ...
  // Fallback when unified_employees view is missing
  ...
  return {
    ...
    /** True when the last fetch returned the limit (suggest using filters to narrow). */
