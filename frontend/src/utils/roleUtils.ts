...
export function canAccessEmployeeData(userRole: AppRole, userDepartment?: string, targetDepartment?: string): boolean {
  // Super admins, executives (CEO, CTO, CFO, COO), admins, and HR can access all employee data
  // Agency owners typically have admin or ceo role
  if (['super_admin', 'ceo', 'cto', 'cfo', 'coo', 'admin', 'hr'].includes(userRole)) {
    return true;
  }
  
  // Operations manager can access all (common for agency setup)
  ...
  // Team leads can only access employees in their department
  if (userRole === 'team_lead' && userDepartment && targetDepartment) {
    return userDepartment === targetDepartment;
  }
