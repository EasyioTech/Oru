...
export async function fetchAgencySettings(agencyDatabase: string): Promise<{ settings: Record<string, unknown> }> {
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({ error: 'Failed to fetch agency settings' }));
    throw new Error(err.error || err.message || 'Failed to fetch agency settings');
  }
  const envelope = await response.json();
  const settings = envelope?.data?.settings ?? envelope?.settings ?? null;
  return { settings };
}
...
export async function updateAgencySettingsApi(
  ...
  const response = await fetch(endpoint, {
    method: 'PUT',
    headers,
    body: JSON.stringify({ settings: payload, database: agencyDatabase }),
  });
  if (!response.ok) {
    const err = await response.json().catch(() => ({ error: 'Failed to update agency settings' }));
    throw new Error(err.error || err.message || 'Failed to update agency settings');
  }
}
