...
function parseOrFilter(filterString: string): { column: string; operator: string; value: string }[] {
  // Parse strings like "col1.eq.val1,col2.ilike.%val2%,col3.contains.val3"
  const parts = filterString.split(',');
  return parts.map(part => {
    const match = part.match(/^(\w+)\.(eq|neq|gt|gte|lt|lte|like|ilike|is|contains)\.(.+)$/);
    if (match) {
      ...
      for (const orFilter of orFilters) {
        ...
        switch (orFilter.operator) {
          ...
          case 'like':
            ...
            break;
          case 'contains':
            // JSONB contains: column @> '["value"]'::jsonb - for arrays of IDs (e.g. assigned_team)
            orCondition = `${orFilter.column} @> $${paramIndex++}::jsonb`;
            params.push(JSON.stringify([orFilter.value]));
            break;
          default:
            continue;
        }
...
const AGENCY_REQUIRED_TABLES = [
  ...
  'dashboard_widgets', 'custom_reports',
...
export function buildInsertStatement(table: string, data: Record<string, any>): { sql: string; params: any[] } {
  ...
  const sql = `INSERT INTO public.${table} (${keys.join(',')}) VALUES (${placeholders})`;
...
export async function insertRecord<T = any>(
  ...
  const { sql, params } = buildInsertStatement(table, { ...data });
  const query = `${sql} RETURNING *`;

  // Pass userId to queryOne so backend can set context in transaction
  const result = await queryOne<T>(query, params, userId);
  if (!result) {
