...
export interface SignInData {
  email: string;
  password: string;
  /** Agency workspace domain for agency login (e.g. acme or acme.app). Required for agency users. */
  domain?: string;
...
/**
 * System-admin (sauth) login – dedicated endpoint, no domain, super_admin only.
 * Use only from the secret /sauth page. Same response shape as loginUser.
 */
export async function loginSauth(data: { email: string; password: string }): Promise<AuthResponse> {
  const apiBaseUrl = getApiBaseUrl();
  const response = await fetch(`${apiBaseUrl}/api/auth/sauth`, {
    method: 'POST',
...
/**
 * Change password (legacy - uses raw SQL; prefer password-service.changePassword for Settings)
 */
export async function changePasswordLegacy(userId: string, oldPassword: string, newPassword: string): Promise<void> {
  const user = await queryOne<User>(
